// Component, Transform, Motion, Health, Sprite, Collider, PlayerController, EnemyAI, BossAI
// ...existing code...
...
class Sprite extends Component {
    ...
    addAnimation(name, frames, speed = 0.1, loop = true) {
        this.animations[name] = { frames: frames, speed: speed, loop: loop, currentFrame: 0, timer: 0 };
    }
    playAnimation(name) {
...
class Collider extends Component {
    ...
    getBounds(transform) {
        ...
        if (this.type === 'circle') {
            return { type: 'circle', x: pos.x, y: pos.y, radius: this.size * Math.max(transform.scale.x, transform.scale.y) };
        } else {
            return { type: 'rectangle', x: pos.x - (this.width * transform.scale.x) / 2, y: pos.y - (this.height * transform.scale.y) / 2, width: this.width * transform.scale.x, height: this.height * transform.scale.y };
        }
    }
}

// ...PlayerController, EnemyAI, BossAI definitions...
// (Move the rest of the component classes here)
